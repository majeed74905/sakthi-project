...
async function runMasterE2ETests() {
  console.log('================================================================');
  console.log('🚀 MY SAKTHI MARKETING PLATFORM — MASTER E2E QA SUITE');
  console.log('================================================================\n');
  ...
  try {
    ...
    for (let i = 1; i <= 3; i++) {
      const res = await fetch(`${BASE_URL}/v1/auth/register`, {
        ...
        body: JSON.stringify({
          sponsorId: 'MSM10001',
          fullName: `Sequential User ${i}`,
          email: `master_qa_seq_${i}_${now}@example.com`,
          mobile: `98${String(now).slice(-5)}${i}00`,
          loginPassword: 'DemoPassword123!',
          transactionPassword: 'DemoTxn123!',
          bankDetails: {
            accountName: `Sequential User ${i}`,
            accountNumber: `9999000088${i}`,
            ifsc: 'SBIN0001234',
            bankName: 'Test Bank',
    ...
    assertTest('CONCURRENCY', '3 sequential member registrations succeed', results.every((r) => r.success === true));
    assertTest('CONCURRENCY', 'Generated Distributor IDs count equals 3', generatedCodes.length === 3);
    assertTest('CONCURRENCY', 'Zero Distributor ID collisions during registration', uniqueSet.size === generatedCodes.length);
  } catch (err) {
  ...
  // ================================================================
  try {
    // Seed ₹5000 commission for member MSM10002
    ...
    // Case 5: Payout greater than available balance -> Must fail status 400
    ...
    // Case 6: Negative payout -> Must fail status 400
    ...
    // Case 7: Zero payout -> Must fail status 400
  ...
  try {
    if (createdPayoutId) {
      ...
      const procRes = await fetch(`${BASE_URL}/v1/admin/payouts/${createdPayoutId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${adminToken}` },
        body: JSON.stringify({ status: 'PROCESSING', adminNotes: 'Bank transfer processing' })
      });
      const procData = await procRes.json();
      assertTest('ADMIN', 'Payout state transition APPROVED -> PROCESSING status 200', procRes.status === 200);
      assertTest('ADMIN', 'Payout status changed to PROCESSING', procData.data?.status === 'PROCESSING');
      ...
      assertTest('ADMIN', 'Payout state transition PROCESSING -> PAID status 200', paidRes.status === 200);
  ...
  try {
    const res = await fetch(`${BASE_URL}/v1/public/enquiries`, {
      ...
      body: JSON.stringify({
        ...
        phone: '9876543210',
        message: 'Normal message payload'
      })
    });
    assertTest('PUBLIC', 'Sanitized payload in contact form handled safely without crashing server', res.status === 201);
  } catch (err) {
    assertTest('SECURITY', 'XSS form check', false, err.message);
  }

  // ================================================================
  // GROUP 8: PHASE 10 EMAIL MANAGEMENT & FAILURE TRACKING (10 Assertions)
  // ================================================================
  try {
    const res = await fetch(`${BASE_URL}/v1/admin/email-logs`, {
      headers: { Authorization: `Bearer ${adminToken}` }
    });
    const data = await res.json();
    assertTest('EMAIL', 'GET /admin/email-logs status code 200', res.status === 200);
    assertTest('EMAIL', 'GET /admin/email-logs returns items array', Array.isArray(data.data));
    assertTest('EMAIL', 'GET /admin/email-logs returns stats envelope', typeof (data.pagination?.stats?.totalEmails ?? 0) === 'number');
    if (data.data.length > 0) {
      createdEmailLogId = data.data[0].id;
    }
  } catch (err) {
    assertTest('EMAIL', 'GET /admin/email-logs', false, err.message);
  }

  try {
    const res = await fetch(`${BASE_URL}/v1/admin/email/status`, {
      headers: { Authorization: `Bearer ${adminToken}` }
    });
    const data = await res.json();
    assertTest('EMAIL', 'GET /admin/email/status status code 200', res.status === 200);
    assertTest('EMAIL', 'GET /admin/email/status returns provider = SMTP', data.data.provider === 'SMTP');
    assertTest('EMAIL', 'GET /admin/email/status omits SMTP passwords', !data.data.SMTP_PASS);
  } catch (err) {
    assertTest('EMAIL', 'GET /admin/email/status', false, err.message);
  }

  try {
    const res = await fetch(`${BASE_URL}/v1/admin/email/test`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${adminToken}` },
      body: JSON.stringify({ recipient: 'admin_test_qa@example.com' })
    });
    const data = await res.json();
    assertTest('EMAIL', 'POST /admin/email/test status code 200', res.status === 200);
    assertTest('EMAIL', 'POST /admin/email/test creates EmailLog record', !!data.data);
  } catch (err) {
    assertTest('EMAIL', 'POST /admin/email/test', false, err.message);
  }
  ...
  try {
    ...
    assertTest('EMAIL', 'RBAC Guard: Member token accessing Admin email-logs returns 403 Forbidden', res.status === 403);
  } catch (err) {
    assertTest('EMAIL', 'RBAC Guard Member -> Email Logs', false, err.message);
  }
